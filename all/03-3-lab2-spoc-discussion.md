...
权限不同，对大部分情况，高特权级可以访问低特权级内容，而相反则是不允许的。

3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。

中断相应：判断当前程序是否有权触发该中断。

函数调用：判断当前程序是否有权调用该函数（包括对函数本身的调用，以及对数据的访问）
...
DPL >= CPL？ 表示当前进程的权限是否满足数据的要求。

DPL >= RPL？ 表示当前进程对段的权限是否满足数据的要求。（注意这里是“对段的权限”而不是其本身的权限）
...
ring0级别的int指令可以触发的中断比ring3级别要多很多，ring3用户权限下只允许触发很少的几个中断，如果在ring3下使用int指令触发其他中断，则会触发一个一般保护异常。

4. 如何利用int和iret指令完成不同特权级的切换？

通过int指令触发中断，可以使特权级进入到ring0态，由操作系统完成一部分工作，然后再通过iret返回ring3态。
...
2次，第一次为段机制映射，第二次为页机制映射。

2. 描述X86-32的MMU地址转换过程；

程序给出的虚拟地址首先通过段机制，查询段表得到段基址，然后结合偏移量得到线性地址；之后通过页机制，通过查页表（可能是多级页表）得到页面地址，结合页偏移量得到物理地址。通过页机制的部分会先查询TLB，如果TLB中没有再查询页表。

### 7.4 了解UCORE建立段/页表

1. 分析MMU的使能过程，尽可能详细地分析在执行进入保护械的代码“movl %eax, %cr0 ; ljmp $CODE_SEL, $0x0”时，CPU的状态和寄存器内容的变化。

修改cr0寄存器后，cr0寄存器的第0位被修改为1，CPU进入保护模式。之后通过长跳转指令，CPU会清空流水线，并重新加载代码段，消除实模式的影响，真正开始按保护模式执行。

2. 分析页表的建立过程；

先申请一个页，用于页目录项，然后设置其映射关系。之后每次需要访问虚拟地址时，如果二级页表中没有对应的项，则创建对应的项。
