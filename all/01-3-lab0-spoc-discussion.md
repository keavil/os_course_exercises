...
- 你理解的对于类似ucore这样需要进程/虚存/文件系统的操作系统，在硬件设计上至少需要有哪些直接的支持？至少应该提供哪些功能的特权指令？
 ...
  硬件设计上应该有层次存储的设计，以及对中断和异常的支持。
  
  应该提供的特权指令包括：中断、中断恢复、读写TLB等，还需要对指令的访存等范围作权限限制。
  
- 你理解的x86的实模式和保护模式有什么区别？物理地址、线性地址、逻辑地址的含义分别是什么？
  
  
  实模式下，cpu运行在16位下，内存至多为1m，同时没有对段的保护；而保护模式下，cpu运行在32位，内存大小为实际内存大小，并启动了对段的保护机制。
  
  
  物理地址：内存中存储的地址
  
  
  线性地址：段内地址，经页表查询后变为物理地址
  
  
  逻辑地址：程序中使用的地址，经段描述符表查询后变为线性地址
